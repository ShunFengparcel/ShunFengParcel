...
type SfPayments struct {
	...
	OrderId   int64     `gorm:"type:int(11);comment:订单id"` // 订单ID
	...
	DeleteAt  time.Time `gorm:"type:datetime;default:NULL;comment:删除时间"` // 删除时间
}
...
func (p *SfPayments) FIndByList(DB *gorm.DB, status string) (List []*payment.PaymentItem, err error) {
	a := DB.Model(p).Select("sf_payments.order_id,sf_payments.pay_no,sf_payments.channel,sf_payments.amount,sf_payments.currency,sf_payments.status,sf_payments.paid_at," +
		"sf_payments.third_tx_id,sf_payments.created_at,sf_users.nickname as UserName").Joins("join sf_users on sf_payments.user_id = sf_users.id")
	if status != "" {
		a = a.Where("status = ?", status)
	}
